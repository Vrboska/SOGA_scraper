...
class DieloPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if type(item).__name__ == 'Dielo':
            self.cursor.execute("select * from diela where id_aukcie=? and id_diela=?",
                                (item['id_aukcie'], item['id_diela']))
            result = self.cursor.fetchone()
            if result:
                log.msg("Item already in database: %s" % item, level=log.DEBUG)
            else:
                self.cursor.execute(
                    "insert into diela (id_aukcie, id_diela, autor, nazov, poradove_cislo, vydrazene,konecna_cena,vyvolavacia_cena,rok,technika,typ_diela,rozmery,url_soga, url_soga_obrazok) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (item['id_aukcie'], item['id_diela'], item['autor'], item['nazov'], item['poradove_cislo'],
                     item['vydrazene'], item['konecna_cena'], item['vyvolavacia_cena'], item['rok'], item['technika'],
                     item['typ_diela'], item['rozmery'], item['url_soga'], item['url_soga_obrazok']))
